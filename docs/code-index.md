...
- [worker/lib/pdf/layout.ts](worker/lib/pdf/layout.ts) — Bloques → páginas: encabezado/pie, tablas paginadas, cajas de firma y la ficha de producto de media hoja (`productCard`) de la OC con imágenes. Exports: renderDocument, wrapText, Block, DocumentMeta, PRODUCT_CARD_TALLAS_MAX.
...
- [worker/lib/pdf/ordenCompraProveedorImagenes.ts](worker/lib/pdf/ordenCompraProveedorImagenes.ts) — Plantilla de la OC a Proveedor CON IMÁGENES: una ficha de media hoja por PRODUCTO (foto grande + tallas), embellecimientos en tabla al final, mismos totales/firmas que la OC normal. Un producto con más de PRODUCT_CARD_TALLAS_MAX tallas se parte en varias fichas — nunca se recorta. Exports: OcProveedorImagenesPdfInput, ProductoGrupo, agruparPorProducto, construirFichas, buildOrdenCompraProveedorImagenesPdf.
