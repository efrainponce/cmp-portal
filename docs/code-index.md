...
- [worker/lib/ocProveedorPdf.ts](worker/lib/ocProveedorPdf.ts) — Arma los datos de la Orden de Compra a Proveedor (folio + líneas agrupadas) y delega el dibujo al escritor de PDF nativo; `conImagenes` elige la plantilla con ficha y foto por producto. Solo lectura, en paralelo al flujo oficial de automations.ts. Exports: OcProveedorPdfError, OcProveedorPdfOpts, generarOcProveedorPdf.
