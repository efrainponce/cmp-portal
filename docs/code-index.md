...
- [shared/telemetry.ts](shared/telemetry.ts) — Contrato de la capa de interacción (`ux_event`): vocabulario de eventos, regex de `target` y saneador de `meta` que el worker usa para VALIDAR y el front para generar. Es la contención ejecutable del guardarraíl "nunca texto capturado por el usuario". Exports: UxKind, UX_KINDS, UX_MAX_BATCH, UX_MAX_DT_MS, UX_RETENTION_DAYS, UX_TARGET_RE, UxEventInput, UxBatch, isValidTarget, isValidUxId, isUxKind, sanitizeMeta, routeSlug.
...
- [worker/lib/telemetry.ts](worker/lib/telemetry.ts) — Ingesta y poda de `ux_event` (capa de interacción, medición de fricción). `user_id` SIEMPRE del identity del servidor; INSERT troceado a 7 filas (84 binds) en un solo batch; retención 90 días. Exports: toRow, ingestUxEvents, purgeUxEvents.
- [worker/lib/uxMetrics.ts](worker/lib/uxMetrics.ts) — Las 5 métricas de fricción comparables contra la línea base de Monday, con el corte PORTAL-vs-MONDAY: `activity_log` no distingue quién originó una edición (el portal escribe a Monday), así que se atribuye cruzando contra `outbox`. Exports: ReEdicionStats, UxReport, Q_ATRIBUCION, Q_AMBIGUOS, Q_CLIC_SIN_ACUSE, Q_REEDICION, Q_TIEMPO_TAREA, Q_ADOPCION, Q_LATENCIA, buildUxReport.
