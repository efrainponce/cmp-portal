...
| PATCH /api/boards/:slug/items/:id | body WriteRequest; every col must pass `canWrite(slug,col,role)` else 403; optimistic: merge into mirror `columns` (text=value), outbox insert, waitUntil flush; ⇒ `{ok:true,pending:true,outboxId}` (optional receipt ID; native D1 writes return `pending:false` without a receipt) |
