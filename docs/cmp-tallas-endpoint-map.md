...
| — | Proyecto creado por el portal (Ganar / crear desde cero) | *(nativo, `worker/lib/drive.ts` crearCarpetaProyectoAuto, flag `DRIVE_PROYECTOS`)* | Proyecto | Desde 2026-09-15 el Proyecto tiene carpeta PROPIA en Drive: `{PRO-nnnn} - {nombre}` bajo "Proyectos Portal" (unidad compartida `0ANaYG13_TI1wUk9PVA`, carpeta `1CXvu__tcLCvb2H0_Wr10srGJbi4AW09I`) con las mismas 12 subcarpetas. Antes `link_mm462saa` heredaba el link de la carpeta de la Oportunidad (ganarOportunidad.ts). Los proyectos anteriores la crean con el botón del tab Documentación | Escribe URL en `link_mm462saa`; el tab Documentación lista subcarpetas/archivos y "Sincronizar documentos" copia a Drive lo que hay en Monday |
