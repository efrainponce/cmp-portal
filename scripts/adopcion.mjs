...
show('Sesiones portal por persona (ux_event 30 d)', d1(`SELECT u.user_id, (SELECT MIN(i.nombre) FROM identity i WHERE i.monday_user_id=u.user_id) nombre, COUNT(DISTINCT substr(u.created_at,1,10)) dias_activos, COUNT(*) eventos FROM ux_event u WHERE u.created_at >= '${DESDE}' GROUP BY u.user_id ORDER BY dias_activos DESC`));
