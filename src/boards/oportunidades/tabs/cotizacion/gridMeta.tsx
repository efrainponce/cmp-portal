...
export function inlineEditableCols(lineEdits: boolean, precio = false, ajusteInline = false): Set<string> {
  ...
  // Compras y admin: color y cantidad SIEMPRE, aunque el board sea de solo
  // lectura para el resto de la línea (Efraín, 2026-08-19) — producto y
  // embellecimiento siguen siendo trabajo de Ventas. El server ya lo permite
  // (shared/visibility.ts `w: V`) y lo asienta como mini versión V{n}.{m} en
  // vez de versionar.
  if (ajusteInline) {
    base.add(COLOR_COL);
    base.add(COL.cantidad);
